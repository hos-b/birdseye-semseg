...
def train(**kwargs):
    ...
    for ep in range(start_ep, epochs):
        ...
        print(f'\nepoch validation loss: {total_valid_t_loss / sample_count} transform')
...
def parse_and_execute():
    ...
    elif train_cfg.classes == 'ours+mask' or train_cfg.classes == 'diminished+mask':
        print('embedded mask should not be used with this script'); exit()
    # snapshot dir -----------------------------------------------------------------------------
    ...
    model.reload_mcnnt3x_checkpoint(train_cfg.extra_model_arg, True)
    ...
    train(train_cfg=train_cfg, device=device, debug_mode=debug_mode, model=model, optimizer=optimizer,
          agent_pool=agent_pool, scheduler=scheduler, transform_loss=transform_loss,
          geom_properties=(new_size, center, ppm), train_loader=train_loader,
          valid_loader=valid_loader, start_ep=start_ep, segmentation_classes=segmentation_classes)
