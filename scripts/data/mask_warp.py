...
def get_centered_img_transform(transforms: torch.Tensor, pixels_per_meter, h, w, center_x, center_y) -> torch.Tensor:
    ...
    rectified_tf[:, 0, 2] = transforms[:, 1, 2] * -pixels_per_meter
    rectified_tf[:, 1, 2] = transforms[:, 0, 2] * -pixels_per_meter
    # image rotation = inverse of cartesian rotation
    rectified_tf[:, :2, :2] = rectified_tf[:, :2, :2].transpose(1, 2)
    porg = torch.tensor([[1.0, 0.0, center_y],
                         [0.0, 1.0, center_x],
                         [0.0, 0.0,      1.0]]).unsqueeze(0)
    norg = torch.tensor([[1.0, 0.0, -center_y],
                         [0.0, 1.0, -center_x],
                         [0.0, 0.0,       1.0]])
