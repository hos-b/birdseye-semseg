...
class MassCNN(torch.nn.Module):
    def __init__(self, config: SemanticCloudConfig, num_classes, conn_drop_prob = 0.01, output_size=(256, 205)):
        ...
        self.mask_prediction = nn.Sequential(
            ...
            LinearBottleneck(in_channels=64, out_channels=64, expansion=4, stride=1),
            # ----------------------------------------------------------------------
    ...
    def forward(self, rgbs, transforms):
        ...
        # [A,   1, 256, 205]
        predicted_masks = F.interpolate(predicted_masks, size=self.output_size, mode='bilinear', align_corners=True)
        # [A, 128, 126, 101]
        ...
        # [A, 128, 504, 404]
        upsampled_features = F.interpolate(pooled_features, size=self.output_size, mode='bilinear', align_corners=True)
        return predicted_masks, self.classifier(upsampled_features)
    ...
    def aggregate_data(self, compressed_features, predicted_masks, transforms):
        ...
        cf_h, cf_w = (126, 101) # compressed_features.shape[2], compressed_features.shape[3]
        ppm = 5.045 # ((cf_h / self.cfg.cloud_x_span) + (cf_w / self.cfg.cloud_y_span)) / 2.0
        center_x = 100 # int((self.cfg.cloud_max_x / self.cfg.cloud_x_span) * cf_h)
        center_y = 50  # int((self.cfg.cloud_max_y / self.cfg.cloud_y_span) * cf_w)
        # aggregating
        aggregated_features = torch.zeros_like(compressed_features)
        for i in range(agent_count):
            relative_tfs = get_relative_img_transform(transforms, i, ppm, cf_h,
                                                      cf_w, center_x, center_y)
